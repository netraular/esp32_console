...
#include <cassert>
#include <map>
#include <cstring> // <-- ¡CORRECCIÓN AQUÍ! Añadir esta línea.
...
static void process_queued_input_cb(lv_timer_t *timer) {
    ...
    if (xQueueReceive(s_input_event_queue, &event_data, 0) == pdPASS) {
        ESP_LOGD(TAG, "[Queued] Executing handler for button %d, event %d", event_data.button_id, event_data.event_type);
...
static void generic_button_event_cb(void *arg, void *usr_data) {
    ...
    // Map the raw event to our internal event type
    auto it = event_map.find(cb_data->raw_event_type);
    if (it == event_map.end()) return; // Should not happen
    button_event_type_t event_type = it->second;

    if (s_current_dispatch_mode == INPUT_DISPATCH_MODE_IMMEDIATE) {
        ESP_LOGD(TAG, "[Immediate] Executing handler for button %d, event %d", button_id, event_type);
        execute_handler(button_id, event_type);
    } else {
        if (s_input_event_queue) {
            ESP_LOGD(TAG, "[Queued] Sending button %d, event %d to queue", button_id, event_type);
...
void button_manager_init() {
    ...
    // Initialize handler structure to all nullptrs
    ...
    // Button config with long press and double click enabled
    button_config_t btn_config = {
        .long_press_time = 1000, // 1 second for long press
        .short_press_time = 300, // 300 ms window for double click
    };
    ...
    // Create each button and register callbacks for all desired event types
